...
exports.generateTagsForText = generateTagsForText;
const https = __importStar(require("https"));
function getGeminiApiKey() {
    const key = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY || null;
    return key && key.trim().length > 0 ? key : null;
}
function buildPrompt(text, contextSnippets = []) {
...
async function generateTagsForText(text, contextSnippets = []) {
        ...
        const req = https.request(options, (res) => {
            ...
            res.on("end", () => {
                try {
                    ...
                    const textParts = parsed?.candidates?.[0]?.content?.parts?.map((p) => p.text || "") || [];
